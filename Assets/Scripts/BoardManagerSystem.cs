...
public class BoardManagerSystem : MonoBehaviour
{
  ...
  public bool behaviourTree = false;
  public TextAsset trainAsset;
  public TextAsset ArcherAsset;
  public TextAsset WarriorAsset;
  public TextAsset RangerAsset;
  ...
  void generateCharactersTraining()
  {
      ...
      if(this.behaviourTree) {
        GameObject.Find("Enemy 1").tag = "BTAgent";
        PandaBehaviour pb = GameObject.Find("Enemy 1").AddComponent<PandaBehaviour>();
        pb.scripts = new TextAsset[1];
        pb.scripts[0] = trainAsset;
        pb.tickOn = Panda.BehaviourTree.UpdateOrder.Manual;
        pb.Apply();
        GameObject.Find("Enemy 1").AddComponent<BlackBoard>().agent = GameObject.Find("Enemy 1").GetComponent<Character>();
      } else {
  ...
  public List<GameObject> createEnemies(Room room, int count)
  {
    ...
    for (int i = 0; i < count; i++)
    {
      ...
      if(behaviourTree) {
        ...
        character.tag = "BTAgent";
        PandaBehaviour pb = character.AddComponent<PandaBehaviour>();
        pb.scripts = new TextAsset[1];
        pb.scripts[0] = asset;
        pb.tickOn = Panda.BehaviourTree.UpdateOrder.Manual;
        pb.Apply();
        character.AddComponent<BlackBoard>().agent = character.GetComponent<Character>();
      } else {
